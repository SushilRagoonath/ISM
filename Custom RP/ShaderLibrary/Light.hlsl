...
Light GetOtherLight (int index, Surface surfaceWS, ShadowData shadowData) {
	...
	float rangeSqr = _OtherLightPositions[index].w;
	// NOTE Sushil: http://learnwebgl.brown37.net/09_lights/lights_attenuation.html
	//float c0 = 1.0f; float c1 = 2.2f; float c2 = 0.2f;
	//float c0 = 0.4f; float c1 = 5.2f; float c2 = 5.2f;
	//float rangeAttenuation = 1.0 / (c0 + c1 * length(ray) + c2 * distanceSqr  );
	//float rangeAttenuation = 2/(distanceSqr + rangeSqr + ray + length(ray)* sqrt(distanceSqr + rangeSqr)   );
	...
	//	spotAttenuation * rangeAttenuation;
		//GetOtherShadowAttenuation(otherShadowData, shadowData, surfaceWS) *
